...
This is the same class of vulnerability as SQL injection, cross-site scripting, and buffer overflows: systems failing to maintain the boundary between data and executable instructions. It is the only threat in this taxonomy that is qualitatively new — everything else is a speed or scale amplification of existing risks.
...
**Research:** [Veracode's 2025 report](https://www.veracode.com/resources/ai-generated-code-security) found that 45% of AI-generated code contains security vulnerabilities from the OWASP Top 10, consistent across over 100 LLMs tested. Sonar's developer survey found that fewer than half of developers review AI-generated code before committing it. [Aikido Security's 2026 report](https://www.aikido.dev/blog/ai-generated-code-is-causing-1-in-5-security-breaches) found that AI-generated code is now the cause of one in five breaches. The pattern is clear: AI-generated code is functional but frequently insecure, and the review process that would catch these issues is often skipped because the code "looks right."
...
In March 2024, researcher [Bar Lanyado](https://vulcan.io/blog/ai-hallucinations-package-risk) asked ChatGPT "How to upload a model to Hugging Face." It generated code referencing a package called `huggingface-cli` — plausible but non-existent (the real package is `huggingface_hub[cli]`). Lanyado registered the name on PyPI as an empty package. Within three months, it had been downloaded over 30,000 times, demonstrating the scale of the exposure.
...
Research from 2023 indicated that 20-35% of hallucinated package names in Python and npm had been converted into actual malicious uploads, though this figure may include benign research registrations.
...
[Wiz research](https://www.wiz.io/blog/wiz-research-uncovers-exposed-ai-development-secrets) (November 2025) found that 65% of the Forbes AI 50 companies had leaked verified secrets on GitHub, including API keys, tokens, and credentials. The most common leak sources were Jupyter Notebook files (.ipynb), Python files (.py), and environment files (.env), with keys primarily from Hugging Face, AzureOpenAI, and WeightsAndBiases. One leaked Hugging Face token could have exposed access to approximately 1,000 private models at a single company. Wiz attributed the problem partly to AI-assisted development practices and attributed secret leakage specifically to "vibe coding" workflows.

In July 2025, a DOGE employee [published a private API key for xAI's language models on GitHub](https://www.gitguardian.com/blog/doge-exposed-xai-api-key), embedded in a script called `agent.py`, granting access to more than 50 xAI LLMs. The key was not detected internally — it was flagged by GitGuardian, an external scanning tool. The key remained live even after public disclosure.

In February 2026, researchers discovered that [nearly 3,000 Google API keys](https://www.apisecurity.io/issue-321-gemini-api-key-leak-gcp-ingress-flaw-oauth-token-theft/), previously considered harmless and commonly embedded in public code, now provided unauthorized access to Google's Gemini AI assistant. The vulnerability emerged when Google introduced Gemini and developers enabled the LLM API on existing projects without realizing the implications — credentials that were benign in their original context became dangerous when the platform expanded around them. This is a perfect example of the "orphaned intermediate state" pattern: the keys were safe when created, but the environment changed and nobody revisited the exposure.
...
**Research:** [Flare researchers](https://flare.io/learn/resources/blog/docker-hub-secrets-leak) (late 2025) found more than 10,000 Docker Hub container images leaking secrets including production API keys, cloud tokens, CI/CD credentials, and AI model access tokens, all pushed into public repositories often unintentionally by developers. [Palo Alto Networks](https://www.paloaltonetworks.com/blog/2025/06/ai-in-the-enterprise/) reported that data loss issues attributed to generative AI more than doubled in early 2025, with AI data security incidents accounting for 14% of all such problems across SaaS traffic.
...
The [Moltbook breach](https://www.wiz.io/blog/moltbook-data-exposure) (late 2025) involved a misconfigured Supabase database — built through vibe coding — that exposed 1.5 million authentication tokens and 35,000 email addresses. The misconfiguration was not the result of a sophisticated attack; it was a development-time setting that was never tightened for production. The site's creator stated he "didn't write one line of code."

**Research:** [Palo Alto Networks' Unit 42 team](https://unit42.paloaltonetworks.com/vibe-coding-security-risks/) (January 2026) documented that a sales lead application was breached because the vibe coding agent neglected to incorporate key security controls such as authentication and rate limiting. The application worked correctly in terms of functionality but was missing the security hardening that a human developer would typically add as a matter of course.
...
The Enrichlead startup launched a platform whose founder publicly stated that 100% of the code was written by Cursor AI with "zero hand-written code." Shortly after launch, security researchers found it riddled with basic security flaws that allowed anyone to access paid features or administrative endpoints. The vulnerabilities were not sophisticated — they were the kind of insecure defaults that a junior developer might produce and a code review would catch.

The [Tea dating app](https://techcrunch.com/2025/07/28/dating-app-tea-suffers-second-data-breach/) suffered two data breaches in 2025, the second exposing approximately 72,000 images including 13,000 government ID photos. While the direct role of AI-generated code versus poor development practices generally is debated (and may be determined in court), the app exemplifies the pattern of rapidly-built applications with fundamental security controls missing.
...
Additionally, researchers revealed in early 2025 that AI training datasets drawn from the public web (notably [Common Crawl](https://www.theregister.com/2025/01/08/ai_training_datasets_contain_thousands/)) contained thousands of valid, active credentials — API keys, passwords, tokens — embedded in publicly accessible code and web pages. LLMs trained on this data may reproduce these credentials in generated code, potentially giving anyone who receives the output access to real systems.
